...
BattleTurn:
	ldh a, [hInMenu]
	push af
	xor a ;ld a, 1 ; or "xor a" for the value 0
	ldh [hInMenu], a
.loop
...
.faint_loop
	call .Function
	ret c
	call HasAnyoneFainted ; fixes a rare glitch of Perish Song + Spikes.
	ret nz
	jr .faint_loop
...
BattleMenu_Pack:
	...
	ld a, [wBattleMode]
	cp TRAINER_BATTLE
    jp nz, .items_can_be_used

	ld a, [wOtherTrainerClass]
	cp OFFICER
	jp c, .ItemsCantBeUsed

	cp SWAT + 1
	jp nc, .ItemsCantBeUsed
...
; Saw this mon
	...
	call CopyBytes
	jmp ApplyStatusEffectOnEnemyStats ; ; Fixes the BRN and PRZ effect on stats that didn't work on switched Pokémons.
...
.transformed
	...
	;callfar BadgeStatBoosts ; Damien
