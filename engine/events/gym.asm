...
; unreferenced ; Damien
MixPalettes::
	ld hl, wBGPals2 palette PAL_OW_PINK ; palette to mix from
	ld de, wBGPals2 palette PAL_OW_GREEN ; palette to edit
	ld bc, 4 ; (1 palettes / 2)
	ld a, BANK(wBGPals2)
...
.CopyByte:
	...
	; division par 2 de chaque couleur (5 bits / composant, stocké sur 2 octets pour les 3 composants soit 15 bits sur 16).
	ld a, [hli]
	ld c, a ; We may need to load b first (instead of c).
	ld a, [hli]
	ld b, a
	push hl ; We save the address of the next byte to copy.

	ld a, [de]
	ld l, a ; We may need to load b first (instead of c).
	inc de
	ld a, [de]
	ld h, a
	dec de
	...
	ld a, h
	ld [de], a
	inc de ; de is the address of the next byte to write on.
	pop hl ; We retrieve the address of the next byte to read.
