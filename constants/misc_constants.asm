...
POKERUS_ORIGINAL_STRAIN EQU POKERUS_XP_DISEASE_MASK | POKERUS_WEAKNESS_DISEASE_MASK
POKERUS_ALPHA_STRAIN EQU POKERUS_DISOBEDIENCE_DISEASE_MASK | POKERUS_WEAKNESS_DISEASE_MASK
POKERUS_DELTA_STRAIN EQU POKERUS_DISOBEDIENCE_DISEASE_MASK | POKERUS_XP_DISEASE_MASK | POKERUS_WEAKNESS_DISEASE_MASK
POKERUS_OMICRON_STRAIN EQU POKERUS_DISOBEDIENCE_DISEASE_MASK | POKERUS_XP_DISEASE_MASK

INCIDENCE_RATE_STRINGBUFFER_OFFSET EQU 17
...
; On dit donc que si le bit est à 1, alors l'objet doit s'afficher. Il faudra quand même faire une repasse globale, pour les objets à activation temporaire.
	...
	const LOCKDOWN			;1
	const CURFEW			;2
	const VACCINE_PASSPORT	;3
	
FREE_F 				EQU %10000 << FREE
LOCKDOWN_F 			EQU %10000 << LOCKDOWN
CURFEW_F 			EQU %10000 << CURFEW
VACCINE_PASSPORT_F 	EQU %10000 << VACCINE_PASSPORT
...
HIDE_FREE				EQU LOW(~FREE_F)
HIDE_LOCKDOWN			EQU LOW(~LOCKDOWN_F)
HIDE_CURFEW				EQU LOW(~CURFEW_F)
HIDE_VACCINE_PASS		EQU LOW(~VACCINE_PASSPORT_F)

START_HOUR_FILTER_OFFSET EQU 4
...
MAX_VITAMIN_STAT_EXP EQU $6400

VIRIDIAN_VERTICAL_WARP_COUNT   EQU 8
VIRIDIAN_HORIZONTAL_WARP_COUNT EQU 4

MEWTWO_MAX_CATCHRATE EQU 25 << 3
MEWTWO_ENCOUNTER_CATCHRATE_BOOST EQU 2 << 3

CAPTURE_MAX_LEVEL_BOOST EQU 15

	const_def
	const NON_METRONOME_USAGE_BIT ; 1
