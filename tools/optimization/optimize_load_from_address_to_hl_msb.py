# optimize_load_from_address_to_hl_msb.py
#
# Optimizes patterns where HL is loaded from two memory addresses into a compact sequence.
# This version expects the high byte (H) to be loaded first.
# Each replacement saves 2 ROM bytes.
#
# Usage:
#   python optimize_load_from_address_to_hl_msb.py          -> shows only the summary
#   python optimize_load_from_address_to_hl_msb.py --full-output  -> also prints each file being optimized
...
# Optimizes a single .asm file by compacting memory-to-HL loads (MSB first).
# Input: Path to a .asm file
# Output: Number of bytes saved (int)
def optimize_load_addr_hl_msb(filepath):
    ...
    while i < len(lines) - 3:
        ...
        match2 = re.match(r'\s*ld\s+h,\s*a(\s*;.*)?$', line2, re.IGNORECASE)
        ...
        if match1 and match2:
            ...
            match4 = re.match(r'\s*ld\s+l,\s*a(\s*;.*)?$', line4, re.IGNORECASE)
...
def optimize_folder(folder_path, full_output=False):
    ...
    for asm_file in asm_files:
        optimizations = optimize_load_addr_hl_msb(asm_file)
        if optimizations > 0:
            if full_output:
