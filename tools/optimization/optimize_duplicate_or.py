# optimize_duplicate_or.py
#
# Removes redundant "or a" (1 byte saved) or "or 0" (2 bytes saved) when it follows another "or XXX".
#
# Usage:
#   python optimize_duplicate_or.py          -> shows only the summary
#   python optimize_duplicate_or.py --full-output  -> also prints each file being optimized
...
# Optimize a single .asm file by removing redundant "or a" or "or 0" after another "or ...".
# Input: Path to .asm file
# Output: Number of bytes saved (int), number of patterns replaced (int)
def optimize_duplicate_or(filepath):
    ...
    while i < len(lines):
        if i < len(lines) - 1:
            m1 = re.match(r'\s*or\s+.+', lines[i], re.IGNORECASE)
            m2 = re.match(r'\s*or\s+(a|0)(\s*;.*)?$', lines[i + 1], re.IGNORECASE)
            if m1 and m2:
                value = m2.group(1).lower()
                saved = 2 if value == '0' else 1
                optimized_lines.append(lines[i])  # keep first "or"
                i += 2
                modified = True
...
def optimize_folder(folder_path, full_output=False):
    ...
    for asm_file in asm_files:
        bytes_saved, patterns = optimize_duplicate_or(asm_file)
        if bytes_saved > 0:
            if full_output:
