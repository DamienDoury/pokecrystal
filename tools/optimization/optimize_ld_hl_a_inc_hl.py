...
#
# Usage:
#   python optimize_ld_hl_a_inc_hl.py          -> shows only the summary
#   python optimize_ld_hl_a_inc_hl.py --full-output  -> also prints each file being optimized
...
# Optimize a single .asm file by replacing ld [hl], a + inc hl with ld [hli], a.
# Input: Path to .asm file
# Output: Number of bytes saved (int), number of patterns replaced (int)
def optimize_ld_hl_a_inc_hl(filepath):
    ...
    while i < len(lines) - 1:
        ...
        m2 = re.match(r'\s*inc\s+hl(\s*;.*)?$', l2, re.IGNORECASE)
        ...
        if m1 and m2:
            ...
            optimized_lines.append(f"{indent}ld [hli], a{comment_suffix}\n")
            i += 2
            modified = True
...
def optimize_folder(folder_path, full_output=False):
    ...
    for asm_file in asm_files:
        bytes_saved, patterns = optimize_ld_hl_a_inc_hl(asm_file)
        if bytes_saved > 0:
            if full_output:
